...
function cleanPhone(phone) {
    ...
    return null; // Возвращаем null для невалидных номеров
}
...
async function refreshAccessToken() {
    ...
    try {
        ...
        refreshToken = jsonResponse.data.refresh_token;
...
function renderLogin() {
    ...
    document.getElementById('login-form').onsubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.user.role !== 'ADMIN') throw new Error('Доступ только для администраторов');
        ...
        districts.forEach(d => {
            html += ` + "`" + `<tr class="border-t">
                ...
                <td class="px-4 py-3 space-x-2">
                    <button data-item='${JSON.stringify(d)}' onclick='editDistrict(JSON.parse(this.dataset.item))' class="text-blue-600 hover:underline text-xs">Изменить</button>
...
async function createDistrict() {
    ...
    const region = document.getElementById('df-region').value.trim();
...
async function createUser() {
    ...
    if (!email || !password || !name || !role || !districtId) {
        alert('Заполните все обязательные поля');
        return;
    }
    ...
    const phoneInput = document.getElementById('uf-phone').value;
    ...
    items.forEach(p => {
        ...
        html += ` + "`" + `<tr class="border-t ${rowColor} hover:bg-blue-50 cursor-pointer" onclick="showPatientDetails(${safe(p.id)})">
            ...
            <td class="px-4 py-3 space-x-2" onclick="event.stopPropagation()">
                <button data-item='${JSON.stringify(p)}' onclick='editPatient(JSON.parse(this.dataset.item))' class="text-blue-600 hover:underline text-xs">Изменить</button>
...
async function createPatient() {
    const firstName = document.getElementById('pf-fname').value.trim();
    const lastName = document.getElementById('pf-lname').value.trim();
    const operationType = document.getElementById('pf-optype').value;
    const eye = document.getElementById('pf-eye').value;

    if (!firstName || !lastName || !operationType || !eye) {
        alert('Заполните все обязательные поля');
        return;
    }

    const phoneInput = document.getElementById('pf-phone').value;
    const phone = cleanPhone(phoneInput);
    if (phoneInput && !phone) {
        alert('Неверный формат телефона. Используйте 10 или 11 цифр.');
        return;
...
async function renderSurgeries(page = 1) {
    ...
    if (!surgeriesResponse.success || !surgeriesResponse.data) {
        throw new Error('Invalid API response');
    }
    if (!patientsResponse.success || !patientsResponse.data) {
        throw new Error('Invalid API response');
    ...
    items.forEach(s => {
        ...
        html += ` + "`" + `<tr class="border-t">
            ...
            <td class="px-4 py-3 space-x-2">
                <button data-item='${JSON.stringify(s)}' onclick='editSurgery(JSON.parse(this.dataset.item))' class="text-blue-600 hover:underline text-xs">Изменить</button>
...
async function createSurgery() {
    ...
    const surgeonId = document.getElementById('sf-surgeon').value;
    const date = document.getElementById('sf-date').value;

    if (!patientId || !surgeonId || !date) {
        alert('Заполните все обязательные поля');
        return;
    }
